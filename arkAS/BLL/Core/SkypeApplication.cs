...
namespace arkAS.BLL.Core
{
    public class SkypeApplication
    {
        ...
        public readonly Skype _skype = new Skype();
        ...
        private SkypeApplication()
        {
            ...
            _skype.MessageStatus += new _ISkypeEvents_MessageStatusEventHandler(OnMessageRevieve); ;
            _hubContext = GlobalHost.ConnectionManager.GetHubContext<BaseHub>();
        }

        private void OnMessageRevieve(ChatMessage pMessage, TChatMessageStatus Status)
        {
            string recievedText;
            string user;
            if (Status == TChatMessageStatus.cmsReceived)
            {
                ...
                recievedText = pMessage.Body;
                _hubContext.Clients.All.getMessage(user, recievedText);
            }
        }
        ...
        public IEnumerable<Contact> ContactList
        {
            get
            {
                ...
                return from User user in friends
                       select new Contact
                       {
    ...
    public static class Extensions
    {
        ...
        public static UserInfo GetUserInfo(this User user)
        {
            ...
            return new UserInfo { Name = user.GetSomeName(), Sex = user.Sex.GetUserInfoSexValue(), login = user.Handle };
        }
